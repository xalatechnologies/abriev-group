...
import { upcoming as upcomingCopy } from "@/content/homePageReference";

const CATEGORY_LABELS: Record<AutoEvent["category"], string> = {
  auction: "Auction",
...
function formatRange(ev: AutoEvent) {
    ...
    <section className="section-y bg-surface font-primary">
      <Container>
        ...
        <div className="grid gap-6 overflow-visible px-0.5 py-2 sm:px-1 md:grid-cols-3 md:gap-8">
          {events.map((ev) => {
            const day = formatDayBadge(ev.date);
            return (
              <Link
                key={ev.id}
                href={`/events/${ev.slug}`}
