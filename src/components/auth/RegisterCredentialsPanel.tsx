...
import { submitRegistration, type RegisterCredState } from "@/actions/auth-register";
import { Link } from "@/i18n/navigation";
import { GoogleGlyph } from "@/components/auth/GoogleGlyph";
import { SurfaceCard } from "@/components/ui/SurfaceCard";

type RegisterCredentialsPanelProps = {
  bannerMessage: string | null;
  oauthReturnPath: string;
};

const initialState: RegisterCredState = null;

export function RegisterCredentialsPanel({
  bannerMessage,
  oauthReturnPath,
}: RegisterCredentialsPanelProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 pb-8">
        <h2 id="register-panel-heading" className="font-headline-lg text-headline-lg text-text-strong">
          {t("panelTitle")}
        </h2>
      ...
      <form action={formAction} className="flex flex-col gap-6">
        <div className="flex flex-col gap-2">
          <label htmlFor={nameId} className="font-label-caps text-label-caps uppercase text-on-surface-variant">
            {t("fullNameLabel")}
          </label>
          <input
            id={nameId}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="relative">
            ...
            <button
              ...
              className="absolute right-2 top-1/2 flex size-9 -translate-y-1/2 items-center justify-center rounded-lg text-text-muted transition-colors hover:bg-surface-container-high hover:text-text-strong focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary/40"
              aria-label={showConfirm ? t("confirmToggleHide") : t("confirmToggleShow")}
      ...
      <p className="mt-8 font-body-md text-[13px] leading-relaxed text-text-muted">
        {t("legalLead")}{" "}
        <Link
          href="/terms"
          className="font-semibold text-brand-primary underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary/40"
        >
          {t("termsLinkLabel")}
        </Link>{" "}
        {t("legalAnd")}{" "}
